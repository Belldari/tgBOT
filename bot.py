...
# ---------------- CONFIG ----------------
TOKEN = "8295318379:AAHCycOMdN_xYlrqp_fZRnVmMancsQCBCfk"        # <- Вставь сюда токен бота
ADMINS = [5174856285]           # <- Список админов
...
async def set_bot_commands():
    commands = [
        ...
        BotCommand(command="give", description="🎟 Выдать билет"),
        BotCommand(command="del_ticket", description="🗑 Удалить билет")
    ]
...
async def cmd_start(message: Message):
    ...
    # Показываем админ-меню если админ
...
# ---------------- Admin commands via buttons + user buttons ----------------
@dp.message()
async def handle_buttons(message: Message):
    text = (message.text or "").strip()
    await add_or_update_user(message.from_user)

    # ADMIN PANEL (buttons inside panel)
    if message.from_user.id in ADMINS:
        # Показать все билеты
        if text == "🎫 Все билеты":
            rows = await get_all_tickets_rows()
            if not rows:
                return await message.answer("🎟 Билетов ещё нет.")
            out = "🎫 *Выданные билеты:*\n\n"
        ...
        # Выдать билет (через команду /give)
        if text == "🎟 Выдать билет":
            return await message.answer("Чтобы выдать билет используйте команду:\n/give user_id")

        # Удалить билет (через команду /del_ticket)
        if text == "🗑 Удалить билет":
            return await message.answer("Чтобы удалить билет используйте команду:\n/del_ticket user_id [ticket_code]")
    ...
    if text == "🎟 Мой билет":
        ...
        return await message.answer("❌ У вас пока нет билета. После оплаты и проверки админ выдаст билет.")

    if text == "📸 Отправить скрин":
        # Новый вариант — просто инструкция
        return await message.answer(
            "📸 Чтобы подтвердить оплату, отправьте скрин админу:\n\n@Belldari"
        )

# ---------------- Admin commands (give / del_ticket) ----------------
...
async def cmd_give(message: Message):
    ...
    # проверка на существующий билет
    ...
    try:
        ...
        await message.answer("⚠ Не удалось отправить сообщение пользователю (возможно он не начинал диалог с ботом).")
