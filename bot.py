...
import aiohttp
from aiohttp import web  # <-- добавлено для фейкового сервера
...
async def start_fake_server():
    port = int(os.getenv("PORT", 8000))  # Render назначает порт в переменной PORT
    app = web.Application()
...
async def main():
    ...
    # keep_alive для Render (если PING_URL указан)
